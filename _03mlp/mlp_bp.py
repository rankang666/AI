...
class Layer:
    ...
    def forward(self,input_data):
        ...
        # 计算加权求和
        # print(self.weights.shape, self.input_data.shape)
...
class MLPNet:
    def __init__(self):
        # 超参数 (网络结构)
        self.paramters = [4,8,6,3]
        self.layers=[]
        for i in range(len(self.paramters)-1):
            layer = Layer(
                input_size=self.paramters[i],
                output_size=self.paramters[i+1],
    ...
    def train(self,samples,labels,times):
        # 循环次数
        for num in range(times):# times
            # 循环样本
            for i in range(samples.shape[0]):
                y = np.array([samples[i]]).T
                # 1. 向前计算输出值(循环神经层)
    ...
    def predict(self,sample):
        ...
        # 判定输出
        # print(v)
        idx = np.argmax(v.reshape(1,self.paramters[-1]))
        return idx
...
# 执行训练
net.train(data,labels,10000)

# 执行准确率测试
counter = 0
for i in range(len(target)):
    r = net.predict(np.array([data[i]]).T)
    if r == target[i]:
        counter += 1
print(f"正确个数:({counter}),正确率:({counter * 100.0/len(target)})")
